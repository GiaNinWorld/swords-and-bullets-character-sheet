...
function Stats() {
    ...
    const updateStatsByAttributes = () => {
        ...
        stats.life = {current: 0, total: (Math.floor(Math.random() * 5) + 1).toString() + '+' + resistenceBonus.toString() + '+' + willBonus.toString()};
        stats.criticalDamage = {current: 0, total: 0};
        stats.fervor = {current: 0, total: willBonus};
        stats.blood = {current: 0, total: resistenceBonus * 2};
        stats.breath = {current: 0, total: resistenceBonus * 2};
        stats.condition = [];
      ...
      }, [data.attributes, data.info.race])
    return (
        <div className="rounded-md bg-blue-100 py-4 overflow-hidden h-full">
            ...
            <ul className='columns-2 gap-0'>
                ...
                <li className='py-2 px-4'>
                    <Input name={'an'} label={'an'} onChange={()=>{}} value={data.stats.an} />
                </li>
                <li className='py-2 px-4'>
