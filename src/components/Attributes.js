...
const Attributes = () => {
  ...
  (<li key={item} className="px-4">
    <div key={item} className='inline-flex w-1/12'>
      <span className='font-bold'>{item.substring(0, 3).toUpperCase()}</span>
    </div>
    <div key={item} className='hidden md:inline-flex md:w-3/12 text-center'>
    ...
    <div className='inline-flex w-1/12 text-center'>{formState[item].modifier}</div>
  </li>)
  );
